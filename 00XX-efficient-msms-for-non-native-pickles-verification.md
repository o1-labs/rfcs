...
As inputs for the MSM algorithm, we take a list of coefficients $`\{c_{i} \}`$, which will have length $n$ equal to `domain_size` for each of the 2 groups, and the set of (SRS) bases $\{G_i\}$. The goal is to compute $\sum_{i=1}^n c_i G_i$ within a kimchi circuit.
