...
**TODO** Potential drawbacks? Even if we create a generic kimchi circuit, we will still need a compatibility wrapper to absorb something else/switch between different representations, so it does not seem to be a concern.

## Rationale and alternatives

**TODO** Perhaps compare with alternative approach of verifying the circuit in zkVM or generically otherwise -- the overhead will be probably significant.

## Prior Art

**TODO** Has anyone implemented a similar wrapper before?

## Unresolved questions

None that are not quite deeply technical. The RFC should be enough to start working on the circuits.
